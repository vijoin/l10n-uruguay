...
class grpSiceSubFamilia(models.Model):

    # Atributos privados
    _name = 'grp.sice_subflia'
    _order = "fami_id, name"
    ...
    name = fields.Char(compute='_concatenar', string=u'Subfamilia', size=60, store=True)
    cod = fields.Integer(string=u'Código de Subfamilia', required=True, index=True)
    descripcion = fields.Char(string=u'Subfamilia', size=60, required=True, index=True)
    ...
    # Restricciones SQL
    _sql_constraints = [
        ("unique_subfamilia", "unique(fami_cod, cod)", u"Código de familia / subfamilia SICE debe ser único"),
    ]
