...
from openerp.addons.account_financial_report_webkit_xls.report.open_invoices_xls \
    import open_invoices_xls
from .open_invoices import PartnersOpenInvoicesWebkitExt
from utils import compute_amounts_in_currency


class open_invoices_xls_ext(open_invoices_xls):
    column_sizes = [12, 12, 20, 15, 30, 30, 14, 14, 14, 14, 14, 14, 14, 14, 14, 10]
    display_alt_curr_columns = None
    base_currency_id = None
    alt_curr_id = -1
    alt_curr_name = None
    alt_curr_context = None

    # Display alternative currency columns
    def display_curr_columns(self, _p, data=None):
        if self.display_alt_curr_columns is None:
            data = data or {}
            ...
            self.alt_curr_id = alt_curr_id
            self.alt_curr_name = alt_curr_id and currency_obj.read(self.cr, self.uid, alt_curr_id, ['name'])['name'] or False
            display_columns = data.get('form',{}).get('display_curr_columns', False) and \
                                    (alt_curr_id and base_currency_id != alt_curr_id or False) or False
            self.display_alt_curr_columns = display_columns
            #self.alt_curr_context = {
            ...
            #}
        return self.display_alt_curr_columns
    ...
    def print_header_titles(self, _p, data, row_position):
        ...
        if data.get('form',{}).get('filter_currency_id', False):
            c_specs += [('fcurr', 2, 0, 'text', _('Currency Filter'),
            None, self.style_bold_blue_center)]
    ...
    def print_header_data(self, _p, data, row_position):
        ...
        if data.get('form',{}).get('filter_currency_id', False):
            c_specs += [('fcurr', 2, 0, 'text', data['form']['filter_currency_id'][1],
            None, self.style_center)]
        if self.display_curr_columns(_p, data):
            c_specs += [('altc', 2, 0, 'text', self.alt_curr_name,
             None, self.style_center)]
        ...
        if self.display_curr_columns(_p, data):
            c_specs += [
                ...
                ('curr_credit', 1, 0, 'text', _('Curr Credit'),
                 None, self.style_yellow_bold_right),
                ('curr_cumul', 1, 0, 'text', _('Curr Cumul. Bal.'),
                 None, self.style_yellow_bold_right),
            ]
    ...
    def print_lines(self, row_position, account, line, _p, data, line_number):
        ...
        if self.display_curr_columns(_p, data):
            ...
            # determine the formula of the cumulated balance
            curr_debit_cell = rowcol_to_cell(row_position, 11)
            curr_credit_cell = rowcol_to_cell(row_position, 12)
            curr_previous_balance = rowcol_to_cell(row_position - 1, 13)
            ...
            # if account has no secundary currency
                ...
                c_specs += [
                    ...
                    ('currcode', 1, 0, 'text', line[
                     'currency_code'] or '', None, style_line_right),
                ]
            #else:
            #    c_specs += [
    ...
    def print_cumul_account(self, row_position, row_start_account, account, _p,
        ...
        if self.display_curr_columns(_p, data):
            curr_range_debit_start = rowcol_to_cell(row_start_account, start_col + 6)
            curr_range_debit_stop = rowcol_to_cell(row_position - 1, start_col + 6)

            curr_range_credit_start = rowcol_to_cell(row_start_account, start_col + 7)
            curr_range_credit_stop = rowcol_to_cell(row_position - 1, start_col + 7)

            curr_total_debit_account = 'SUMIF(' + reference_start + ':' + \
                reference_stop + ';"' + search_key + '";' + curr_range_debit_start + \
                ':' + curr_range_debit_stop + ')'
            curr_total_credit_account = 'SUMIF(' + reference_start + ':' + \
                reference_stop + ';"' + search_key + '";' + curr_range_credit_start + \
                ':' + curr_range_credit_stop + ')'

            curr_bal_account_debit = rowcol_to_cell(row_position, start_col + 6)
            curr_bal_account_credit = rowcol_to_cell(row_position, start_col + 7)
            curr_bal_account_total = curr_bal_account_debit + '-' + curr_bal_account_credit

            c_specs += [
                ('curr_debit', 1, 0, 'number', None,
                 curr_total_debit_account, self.style_account_title_decimal),
                ('curr_credit', 1, 0, 'number', None,
                 curr_total_credit_account, self.style_account_title_decimal),
                ('curr_balance', 1, 0, 'number', None,
                 curr_bal_account_total, self.style_account_title_decimal),
            ]
    ...
    def print_group_cumul_account(self, row_position, row_start_account,
        ...
        if self.display_alt_curr_columns:
            curr_range_debit_start = rowcol_to_cell(row_start_account, start_col + 6)
            curr_range_debit_stop = rowcol_to_cell(row_position - 1, start_col + 6)
    ...
    def generate_xls_report(self, _p, _xs, data, objects, wb):  # main function **
        if self.localcontext:
            self.localcontext.update({'lang': (_p.get('lang') or 'en_US')})
        else:
            self.localcontext = {'lang': (_p.get('lang') or 'en_US')}
        # Initializations
        ...
        # TODO: FIXME This solution must be generic, not only for XLS report
        # If we override set_context to set attribute 'grouped_ledger_lines'
        # in every account, then in this function account objects don't have
        # attribute 'grouped_ledger_lines'. Trace it to find a new solution ?
        group_by_currency = data['form'].get('group_by_currency', False)
        if group_by_currency:
            parser = PartnersOpenInvoicesWebkitExt(self.cr, self.uid, '', {})
        ## END

        for acc in objects:
            ## BEGIN
            if group_by_currency:
                acc.ledger_lines = _p.get('ledger_lines', {}).get(acc.id, {})
                parser._group_lines_by_currency(acc)
            ## END
...
open_invoices_xls_ext('report.account.account_report_open_invoices_xls_ext',
                        'account.account', parser=PartnersOpenInvoicesWebkitExt)
