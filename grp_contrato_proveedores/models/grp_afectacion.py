...
class grpContratoAfectacion(models.Model):
    ...
    def _compute_contrato_id(self):
        for rec in self:
	 # TODO: K INCIDENCIA
            contratos = self.env['grp.contrato.proveedores'].search([('convenio', '=', True), ('contrato_general_id', '=', False)]).filtered(lambda x: rec.id in x.afectaciones_ids.ids)
            if contratos:
                rec.contrato_id = contratos[0].id

    contrato_id = fields.Many2one('grp.contrato.proveedores', compute='_compute_contrato_id', string=u'Nro Contrato',
                                  readonly=True)
