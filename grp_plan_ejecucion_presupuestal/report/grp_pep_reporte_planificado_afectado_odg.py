...
class grp_pep_reporte_planificado_afectado_odg(models.AbstractModel):
    _name = 'grp.pep.reporte.planificado.afectado.odg'
    ...
    def generar_excel(self, wizard):
            ...
            for concepto in conceptos:
                ...
                for llave in historia_llaves:
                    if llave.plan_periodo <= version:
                        clave = llave.llave_str
                        if datos.get(clave):
                            if datos[clave].get('periodo_afectado_' + str(llave.plan_periodo)):
                                datos[clave]['periodo_afectado_' + str(llave.plan_periodo)] += llave.importe
                            else:
                                datos[clave]['periodo_afectado_' + str(llave.plan_periodo)] = llave.importe
        ...
        for key, value in datos.iteritems():
            lista.append(value)
        lineas = sorted(lista, key=lambda item: (item['llave_odg'], item['llave_aux']))
        ...
        # Escribo el titulo
        ws.write_merge(fila, 0, fila, 17, u"COMPARACIÓN IMPORTE PLANIFICADO VERSUS IMPORTE AFECTADO POR OBJETO DEL GASTO", title_left_bloqueado)
        fila = fila + 1
        ...
        # Cabezal
        fila = fila + 2
        ws.write(fila, 0, u"ODG", header_left_bloqueado)
        ws.write(fila, 1, u"Aux", header_left_bloqueado)
        ...
        ws.write(fila, 9, u"Importe anual planificado", header_left_bloqueado)
        ws.write(fila, 10, u"Importe anual afectado", header_left_bloqueado)
        for i in range(0, tope-1):
            ws.write(fila, 11 + i*2, u'Período ' + str(i+1) + u' planificado', header_left_bloqueado)
            ws.write(fila, 11 + (i*2)+1, u'Período ' + str(i+1) + u' afectado', header_left_bloqueado)
        fila = fila + 1
        # Líneas
        for linea in lineas:
            ws.write(fila, 0, linea['llave_odg'], texto_editable)
            ws.write(fila, 1, linea['llave_aux'], texto_editable)
            ...
            ws.write(fila, 9, linea['importe_anual_planificado'], integer_editable)
            ws.write(fila, 10, linea['importe_anual_afectado'], integer_editable)
            for i in range(0, tope - 1):
                ws.write(fila, 11 + i*2, linea['periodo_planificado_' + str(i+1)], integer_editable)
                ws.write(fila, 11 + (i*2)+1, linea['periodo_afectado_'+str(i+1)], integer_editable)
            fila = fila + 1
        ...
        fp.close()
        file_name = 'Export_planificado_vs_afectado_odg_' + plan.name + '.xls'
    ...
    def _get_lineas_aprobado(self, plan_id):
        ...
        for linea_gasto in plan.lineas_gasto:
            ...
            for gasto in concepto.lineas_gasto:
                clave = gasto.display_name
                if not lineas.get(clave):
                    lineas[clave] = ({'importe_anual_planificado': 0,
                ...
                for i in range(1, periodicidad+1):
                    ...
                    if lineas[clave].get('periodo_planificado_' + str(i)):
                        lineas[clave]['periodo_planificado_' + str(i)] += importe_periodo
                    else:
                        lineas[clave]['periodo_planificado_' + str(i)] = importe_periodo
        return lineas

grp_pep_reporte_planificado_afectado_odg()
